...
{
    return RTN_alloc(NULL, 0, 0);
}

const CHAR *RTN_Name (RTN rtn)
{
    lsassert(RTN_Valid(rtn));
    return rtn.name;
}
...
RTN RTN_FindByAddress(ADDRINT address)
{
    ...
    return RTN_alloc(sym->name, address, sym->size);
}
...
/*GLP: I think it should be moved to ins_instrument*/
...
ANALYSIS_CALL *RTN_get_exit_cbs(uintptr_t pc, int *cnt)
{
    ...
    /* FIXME 目前假设所有rtn不重叠，因此pc只出现在一个rtn的范围中 */
