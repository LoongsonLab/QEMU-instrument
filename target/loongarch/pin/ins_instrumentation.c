 ...
 * 解析IARG，记录到callback结构�? * 只支持整数类型的参数，最�?�? * 只做最基本的“词法分析”，不做语法检�? */
...
/* 调用分析函数前，需要保存的寄存器�? * 包括：直接映射到了“调用者保存寄存器”的寄存器，以及sp,tp
 * TODO: reg_tp 要保存，要恢复成host的吗？我觉得�?*/
...
{
    /* 若对同一指令插入多个分析函数调用，我们只需在这些调用前后保存和恢复一次caller-saved寄存�?*/
...
static Ins *ins_get_next_cb_position(INS INS, IPOINT ipoint)
{
    /* 若对同一指令插入多个分析函数调用，我们只需在这些调用前后保存和恢复一次caller-saved寄存�?*/
    ...
    if (ipoint == IPOINT_AFTER) {
        lsassertm(INS_IsValidForIpointAfter(INS), "can not instrument at IPOINT_AFTER for unconditional branch\n");
        if (INS->iafter_next_cb == NULL) {
            // 在INS末尾临时插一条nop，方便在nop之前插入指令
            INS_insert_ins_after(INS, INS->last_ins, ins_nop());
            INS->iafter_next_cb = init_callback_block(INS, INS->last_ins);
    ...
    if (ipoint == IPOINT_TAKEN_BRANCH) {
        lsassertm(INS_IsValidForIpointTakenBranch(INS), "can not instrument at IPOINT_TAKEN_BRANCH for non-branch\n");
       if (INS->iafter_next_cb == NULL) {
             INS_insert_ins_after(INS, INS->last_ins, ins_nop());
            INS->iafter_next_cb = init_callback_block(INS, INS->last_ins);
            INS_remove_ins(INS, INS->last_ins);
...
static void insert_callback(INS INS, Ins *cur, ANALYSIS_CALL *cb)
{
    ...
    /* 调用分析函数后的触发的事情，如IARG_REG_REFERENCE修改的寄存器需要写�?*/
...
VOID INS_InsertCall(INS INS, IPOINT ipoint, AFUNPTR funptr, ...)
{
    ...
    Ins *cur = ins_get_next_cb_position(INS, ipoint);
...
VOID INS_InsertIfCall(INS INS, IPOINT ipoint, AFUNPTR funptr,...)
{
    ...
    /* 此处不插桩，推迟到INS_InsertThenCall()再插�?*/
    va_list valist;
    va_start(valist, funptr);
...
VOID INS_InsertThenCall (INS INS, IPOINT ipoint, AFUNPTR funptr,...)
{
    ...
    /* 目前的设计是将IfCall的返回值放�?a0里，根据$a0决定是否调用thenCall */
    ...
    Ins *jmp = NULL;
    int offset_opnd_index = -1;
    if(PIN_instru_ctx.ins_predicate_valid == true)
    {
    /* 根据指令的predicate决定是否跳过分析函数 */
    IR2_OPCODE op = INS->origin_ins->op;
    switch (op) {
    ...
    /* 若ifCall返回�?即reg_a0值为0，则跳过funptr分析调用*/
    ...
    /* 修正jmp,beqz跳过的指令数 */
    int offset = 0;
    if(PIN_instru_ctx.ins_predicate_valid == true && jmp!= NULL)
    {
        Ins *i = jmp;
        while (i != cur) {
            ++offset;
    ...
    PIN_instru_ctx.ins_predicate_valid = false;
}

VOID INS_InsertIfPredicatedCall (INS INS, IPOINT ipoint, AFUNPTR funptr,...)
{
    lsassertm(ipoint == IPOINT_BEFORE || INS_HasFallThrough(INS), "can not instrument at IPOINT_AFTER for unconditional branch\n");

    /* 此处不插桩，推迟到ThenCall()再插�?*/
    va_list valist;
    va_start(valist, funptr);
    PIN_instru_ctx.ins_if_call_cb = parse_iarg(IOBJECT_INS, ipoint, funptr, valist);
    PIN_instru_ctx.ins_if_call_valid = true;
    PIN_instru_ctx.ins_predicate_valid = true;
    va_end(valist);
}

VOID INS_InsertThenPredicatedCall (INS INS, IPOINT ipoint, AFUNPTR funptr,...)
{
    lsassertm(PIN_instru_ctx.ins_if_call_valid == true && PIN_instru_ctx.ins_if_call_cb.ipoint == ipoint, "should call IfCall before ThenCall\n");
    ANALYSIS_CALL *if_cb = &PIN_instru_ctx.ins_if_call_cb;
    PIN_instru_ctx.ins_if_call_valid = false;
    PIN_instru_ctx.ins_predicate_valid = false;
...
{
    /* FIXME more strict validation */
    /* 目前的设计是ThenCall必须紧接着IfCall调用，因为IfCall的返回值放�?a0�?*/
...
VOID TRACE_InsertCall(TRACE trace, IPOINT ipoint, AFUNPTR funptr, ...)
{
    lsassertm(ipoint == IPOINT_BEFORE || TRACE_HasFallThrough(trace), "can not instrument at IPOINT_AFTER for unconditional branch\n");

    va_list valist;
    va_start(valist, funptr);
    ANALYSIS_CALL cb = parse_iarg(IOBJECT_TRACE, ipoint, funptr, valist);
    va_end(valist);
...
VOID TRACE_InsertIfCall(TRACE trace, IPOINT ipoint, AFUNPTR funptr, ...)
{
    ...
    va_list valist;
    va_start(valist, funptr);
    PIN_instru_ctx.trace_if_call_cb = parse_iarg(IOBJECT_TRACE, ipoint, funptr, valist);
    PIN_instru_ctx.trace_if_call_valid = true;
    va_end(valist);
}
...
VOID TRACE_InsertThenCall (TRACE trace, IPOINT ipoint, AFUNPTR funptr, ...)
{
    ...
    PIN_instru_ctx.trace_if_call_valid = false;
...
{
    if(rtn->entry_cbs_num == 0)
    {
        rtn->rtn_entry_cbs = (ANALYSIS_CALL *) malloc(sizeof(ANALYSIS_CALL) * MAX_CB_NUM);
    }
    rtn->rtn_entry_cbs[rtn->entry_cbs_num ++] = *cb;
}

/*记录函数出口处待插入的分析调�*/
static VOID RTN_add_exit_cb(RTN rtn, ANALYSIS_CALL *cb)
{
    if(rtn->exit_cbs_num == 0)
    {
        rtn->rtn_exit_cbs = (ANALYSIS_CALL *) malloc(sizeof(ANALYSIS_CALL) * MAX_CB_NUM);
    }
    rtn->rtn_exit_cbs[rtn->exit_cbs_num ++] = *cb;
}
...
static ANALYSIS_CALL *RTN_get_entry_cbs(uintptr_t pc, int *cnt)
{
    for(int i = 1; i <= IMG_id; i++)
        ...
        {
            if(RTN_array[i][j].addr == pc && RTN_array[i][j].entry_cbs_num != 0)
            {
                *cnt = RTN_array[i][j].entry_cbs_num;
                return RTN_array[i][j].rtn_entry_cbs;
            }
        }
    *cnt = 0;
    return NULL;
...
static ANALYSIS_CALL *RTN_get_exit_cbs(uintptr_t pc, int *cnt)
{
    for(int i = 1; i <= IMG_id; i++)
        for(int j = 1; j <= IMG_array[IMG_id].rtn_num; j++)
        {
            ...
            {
                if(RTN_array[i][j].exit_cbs_num != 0)
                {
                    *cnt = RTN_array[i][j].exit_cbs_num;
                    return RTN_array[i][j].rtn_exit_cbs;
                }
            }
        }
    *cnt = 0;
...
/*根据Trace的出入口是否是RTN的出入口，对Trace插入分析调用*/
VOID RTN_instrument(TRACE trace)
{
    ...
    int cb_cnt;
    ANALYSIS_CALL *entry_cbs = RTN_get_entry_cbs(ins->pc, &cb_cnt);
    for (int i = 0; i < cb_cnt; ++i) {
        _RTN_InsertCall(ins, &entry_cbs[i]);
    }
    //free(entry_cbs);
    ...
    if (origin_ins->op == LISA_JIRL &&
        ...
        ANALYSIS_CALL *exit_cbs = RTN_get_exit_cbs(ins->pc, &cb_cnt);
        for (int i = 0; i < cb_cnt; ++i) {
            _RTN_InsertCall(ins, &exit_cbs[i]);
        }
        //free(exit_cbs);
    }
}
